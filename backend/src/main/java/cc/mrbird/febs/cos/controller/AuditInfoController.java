...
public class AuditInfoController {
    ...
    public R save(AuditInfo auditInfo) throws FebsException {
        StudentInfo studentInfo = studentInfoService.getOne(Wrappers.<StudentInfo>lambdaQuery().eq(StudentInfo::getUserId, auditInfo.getOwnerId()));
        if (studentInfo == null) {
            throw new FebsException("该用户不存在");
        }
        // 判断用户是否有在待审核的申请
        if (auditInfoService.count(Wrappers.<AuditInfo>lambdaQuery().eq(AuditInfo::getOwnerId, studentInfo.getId()).eq(AuditInfo::getStatus, "0")) > 0) {
            throw new FebsException("该用户有未审核的申请，请勿重复申请");
        }
        auditInfo.setOwnerId(studentInfo.getId());
