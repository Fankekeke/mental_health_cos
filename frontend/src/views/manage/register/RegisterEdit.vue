<template>
  <a-modal v-model="show" title="修改辅导预约" @cancel="onClose" :width="800">
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        <a-col :span="12">
          <a-form-item label='辅导预约姓名' v-bind="formItemLayout">
            <a-input v-decorator="[
            'name',
        ...
        <a-col :span="12">
          <a-form-item label='辅导预约性别' v-bind="formItemLayout">
            <a-select v-decorator="[
              'sex',
              { rules: [{ required: true, message: '请输入辅导预约性别!' }] }
              ]">
...
export default {
  ...
  computed: {
    ...
    show: {
      get: function () {
        return this.registerEditVisiable
      },
      set: function () {
      }
    }
    ...
    setFormValues ({...register}) {
      ...
      Object.keys(register).forEach((key) => {
        if (key === 'sex' || key === 'status' || key === 'isAdmin') {
          register[key] = register[key].toString()
        }
        if (key === 'images') {
          this.fileList = []
          this.imagesInit(register['images'])
        }
        if (fields.indexOf(key) !== -1) {
          this.form.getFieldDecorator(key)
          obj[key] = register[key]
        }
        if (key === 'deptId' && register['deptId'] != null && register['hospitalName'] != null) {
          this.fetchUser(register['hospitalName'])
        }
      })
