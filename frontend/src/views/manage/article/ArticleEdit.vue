<template>
  <a-modal v-model="show" title="修改文章" @cancel="onClose" :width="800">
...
export default {
  ...
  computed: {
    ...
    show: {
      get: function () {
        return this.articleEditVisiable
      },
      set: function () {
      }
    }
  ...
  methods: {
    ...
    setFormValues ({...article}) {
      this.rowId = article.id
      let fields = ['title', 'content', 'createBy']
      let obj = {}
      Object.keys(article).forEach((key) => {
        if (key === 'images') {
          this.fileList = []
          this.imagesInit(article['images'])
        }
        if (key === 'type' && article[key] != null) {
