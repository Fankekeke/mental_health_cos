...
<!--    </schedule-edit>-->
    <schedule-view
      ...
      :scheduleData="scheduleView.data">
    </schedule-view>
  </a-card>
</template>
...
export default {
  name: 'schedule',
  components: {scheduleAdd, scheduleEdit, scheduleView, RangeDate},
  data () {
    return {
      advanced: false,
      scheduleAdd: {
  ...
  methods: {
    handleViewOpen (row) {
      this.scheduleView.data = row
      this.scheduleView.visiable = true
    },
    handleViewClose () {
      this.scheduleView.visiable = false
    },
    editStatus (row, status) {
      this.$post('/cos/schedule-info/account/status', { scheduleId: row.id, status }).then((r) => {
    ...
    add () {
      this.scheduleAdd.visiable = true
    },
    handlescheduleAddClose () {
      this.scheduleAdd.visiable = false
    },
    handlescheduleAddSuccess () {
      this.scheduleAdd.visiable = false
      this.$message.success('新增排班成功')
      this.search()
    },
    edit (record) {
      this.$refs.scheduleEdit.setFormValues(record)
      this.scheduleEdit.visiable = true
    },
    handlescheduleEditClose () {
      this.scheduleEdit.visiable = false
    },
    handlescheduleEditSuccess () {
      this.scheduleEdit.visiable = false
