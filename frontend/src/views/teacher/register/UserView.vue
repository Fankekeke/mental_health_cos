<template>
  <a-modal v-model="show" title="学生辅导" @cancel="onClose" :width="1200">
      ...
      <a-col :span="16">
        <div class="user-info-container">
          ...
          <a-row :gutter="16" class="info-content">
            <a-col :span="8">
              <div class="info-item">
                <span class="info-label">学生名称：</span>
                <span class="info-value">{{ userData.studentName }}</span>
              </div>
            </a-col>
            <a-col :span="8">
              <div class="info-item">
          ...
          <div class="section-header" style="margin-top: 30px;">
            ...
            <span class="section-title">辅导记录</span>
          </div>
              ...
              <template slot="contentShow" slot-scope="text, record">
                <template>
                  <a-tooltip>
                    ...
                    <span class="cause-text">{{ record.remark.slice(0, 15) }} ...</span>
                  </a-tooltip>
                </template>
              </template>
            </a-table>
      ...
      <a-col :span="8">
        <div class="ai-analysis-section">
          <div class="ai-header">
            ...
            <span class="ai-title">AI辅导分析</span>
          </div>
...
export default {
  ...
  watch: {
    userShow: function (value) {
      if (value) {
        this.$get(`/cos/register-info/byUser/${this.userData.userId}`).then((r) => {
          this.durgList = r.data.data
          let message = ''
          if (this.durgList.length !== 0) {
            this.durgList.forEach((item) => {
              message += '时间：' + item.createDate + ', 备注：' + item.symptom + ', 辅助内容：' + item.remark + '\n'
            })
  ...
  methods: {
    ...
    generateAiAnalysis () {
      if (this.durgList.length === 0) {
        this.$message.warning('暂无辅导记录可供分析')
        return
      }
      ...
      let message = '请对以下辅导进行分析并给出建议（300字内）：\n'
      this.durgList.forEach((item) => {
        message += '时间：' + item.createDate + ', 备注：' + item.symptom + ', 辅助内容：' + item.remark + '\n'
      })
